...
func (m GridMove) CrossNoBackward() [3]GridMove {
	switch m {
	case MoveUp:
		return [3]GridMove{MoveUp, MoveRight, MoveLeft}
	case MoveRight:
		return [3]GridMove{MoveRight, MoveUp, MoveDown}
	case MoveDown:
		return [3]GridMove{MoveDown, MoveRight, MoveLeft}
	case MoveLeft:
		return [3]GridMove{MoveLeft, MoveUp, MoveDown}
	default:
		panic(fmt.Sprintf("invalid move for cross no backward %v", m))
	}
}
