...
func (m GridMove) String() string {
	...
	case MoveDownRight:
		return "DownRight"
	default:
		return fmt.Sprintf("Unknown (%d, %d)", m[0], m[1])
	}
}

func (m GridMove) DirectionRune() rune {
	switch m {
	case MoveUp:
		return '^'
	case MoveRight:
	...
	case MoveDownRight:
		return '↘'
	default:
		panic(fmt.Sprintf("invalid move for direction rune %v", m))
	}
}
...
func (m GridMove) Multiply(n int) GridMove {
	return GridMove{m[0] * n, m[1] * n}
}
...
func (p Point[T]) LessCoordinates(other Point[T]) bool {
...
// Up, Right, Down, Left
func (g *Grid[T]) GetNeighborsCrossNoWalls(row, column int, iswall func(T) bool) []Point[T] {
	neighbors := make([]Point[T], 0, 4)

	for _, move := range GridCrossMoves {
		newRow, newCol := row+move[0], column+move[1]

		if g.IsInside(newRow, newCol) && !iswall(g.Get(newRow, newCol).Value) {
			n := g.Get(newRow, newCol)
			neighbors = append(neighbors, n)
		}
	}
...
func (g *Grid[T]) GetByMoveCheck(row, column int, move GridMove) (Point[T], bool) {
	if !g.IsInside(row, column) {
		panic(
			fmt.Sprintf(
...
func (g *Grid[T]) ToIndex(row, col int) int {
...
func NewSparseGrid[T any](rows, columns int) *SparseGrid[T] {
	return &SparseGrid[T]{
		Rows:       make([][]Point[T], rows),
		Columns:    make([][]Point[T], columns),
		NumRows:    rows,
		NumColumns: columns,
	}
}

func (g *SparseGrid[T]) Add(row, column int, value T) {
	point := Point[T]{Row: row, Col: column, Value: value}
	g.AddPoint(point)
}

func (g *SparseGrid[T]) AddPoint(point Point[T]) {
	g.Rows[point.Row] = binInsertPoint(g.Rows[point.Row], point)
	g.Columns[point.Col] = binInsertPoint(g.Columns[point.Col], point)
}

func (g *SparseGrid[T]) RemovePoint(point Point[T]) {
	row := g.Rows[point.Row]
	i, found := slices.BinarySearchFunc(row, point, LessPoint)
	if !found {
		return
	}
	g.Rows[point.Row] = append(row[:i], row[i+1:]...)
}

func (g *SparseGrid[T]) FindPoint(row, column int) (Point[T], bool) {
	toSearch := g.Rows[row]
	toSearchCol := g.Columns[column]
	if len(toSearchCol) < len(toSearch) {
		toSearch = toSearchCol
	}

	i, found := slices.BinarySearchFunc(toSearch, Point[T]{Row: row, Col: column}, LessPoint)
	if found {
		return toSearch[i], true
	}
	return Point[T]{}, false
}

func (g *SparseGrid[T]) Copy() *SparseGrid[T] {
	rows := make([][]Point[T], g.NumRows)
	for i, row := range g.Rows {
		rows[i] = make([]Point[T], len(row))
		copy(rows[i], row)
	}
	columns := make([][]Point[T], g.NumColumns)
	for i, col := range g.Columns {
		columns[i] = make([]Point[T], len(col))
		copy(columns[i], col)
	}
...
func LessPoint[T any](a, b Point[T]) int {
	if a.Row < b.Row {
		return -1
	}
	if a.Row > b.Row {
...
func binInsertPoint[T any](slice []Point[T], point Point[T]) []Point[T] {
	i, found := slices.BinarySearchFunc(slice, point, LessPoint)
	if found {
		return slice
	}

	return slices.Insert(slice, i, point)
}
